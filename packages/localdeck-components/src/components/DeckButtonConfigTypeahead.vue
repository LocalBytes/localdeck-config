<template>
  <div ref="inputRef" :class="{'dropdown dropdown-open':isOpen}" class="w-full"
       @focusout="isOpen=false"
       @focusin.once="isOpen=true"
  >
    <input v-model="modelValue" class="input w-full" type="text" @input="isOpen=true">
    <div class="dropdown-menu dropdown-menu-bottom-right gap-1 w-auto max-h-[400px] overflow-y-auto">
      <div
          ...
          @click="select(item)"
      >
        <span class="text-lg" v-html="renderString(item.name,modelValue)"/>
        <span v-html="renderString(item.id, modelValue)"/>
      </div>
      <div v-if="filtered.length==0" class="dropdown-item disabled">
