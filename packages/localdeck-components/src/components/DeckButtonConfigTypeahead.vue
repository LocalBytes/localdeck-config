  ...
  >
    ...
    >
      <button
        ...
        @click="select(item)"
      >
        <component :is="renderString(item.name, modelValue)" class="block" />
        <component :is="renderString(item.id, modelValue)" class="block" />
      </button>
      <div v-if="filtered.length === 0" class="px-3 py-2">No results</div>
    </div>
  </div>
</template>
