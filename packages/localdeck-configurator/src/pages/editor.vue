<template>
  <div class="container py-5 mx-auto">
    ...
    <div class="form-control my-2 *:m-1">
      ...
      <button class="btn btn-outline btn-primary" type="button" @click="print">Print</button>
    </div>
    <div class="flex flex-wrap justify-center gap-10 py-2">
      ...
      <div v-else class="basis-2 grow" />
    </div>

    <LbUiModal v-model="resetting" title="Are you sure?">
      <p>
        This will reset all button configurations to their default values.<br />
        It will <b>not</b> save your changes by itself, so you will need to click the save button
        after resetting!
      </p>
      <div class="flex justify-end gap-2">
        <button class="btn btn-error btn-outline" @click="reset">Reset</button>
    ...
    <LbUiModal
      ...
      title="Saving..."
      @update:model-value="
        (value: boolean) => (saving = value ? SavingStatus.DONE : SavingStatus.IDLE)
      "
    >
      ...
      <div v-else>
        <p>Your changes have been saved, please go to ESPHome to install!</p>
      </div>
    </LbUiModal>
  </div>
</template>
...
const { data, status } = await useServerFetch("/api/editor", {
